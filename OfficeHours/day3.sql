-- 1. write a SQL query to find the first name, last name,
-- department number, and department name for each employee.
--you need to identify which dept you wanna display, as we got
-- similar columns DEPARTMENT_ID in EMPLOYEES and DEPARTMENTS tables
-- that's why assignment at line 4
SELECT EMPLOYEES.FIRST_NAME, EMPLOYEES.LAST_NAME, DEPARTMENTS.DEPARTMENT_ID, DEPARTMENTS.DEPARTMENT_NAME
FROM EMPLOYEES
INNER JOIN DEPARTMENTS ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID;

-- 2. write a SQL query to find the first name, last name,
-- department name, city, and state province for each employee.
...
FROM EMPLOYEES INNER JOIN JOB_GRADES
ON EMPLOYEES.SALARY BETWEEN JOB_GRADES.LOWEST_SAL AND JOB_GRADES.HIGHEST_SAL;

-- 4. write a SQL query to find all those employees who work in
-- department ID 80 or 40. Return first name, last name,
...
-- 5. write a SQL query to find those employees whose first
-- name contains a letter ‘z’. Return first name, last name,
-- department name, city, and state province.
...
-- 8. write a SQL query to display the department name, city,
...
LEFT OUTER JOIN (SELECT *FROM EMPLOYEES
                ...
                (SELECT DISTINCT MANAGER_ID FROM EMPLOYEES
                 WHERE MANAGER_ID IS NOT NULL)) M ON M.EMPLOYEE_ID = E.MANAGER_ID;

-- 12.write a SQL query to find those employees who work in a department
-- where the employee of last name 'Taylor' works.
...
-- 13.write a SQL query to find those employees who get higher salary
-- than the employee whose ID is 163. Return first name, last name.
SELECT FIRST_NAME, LAST_NAME, SALARY
FROM EMPLOYEES
WHERE SALARY>(
...
-- 15.write a SQL query to find those employees whose salary matches
...
-- 17. write a SQL query to find those employees who report that manager
-- whose first name is ‘Payam’.
...
-- 19. write a SQL query to find those employees whose ID matches
-- any of the number 134, 159 and 183. Return all the fields.
SELECT *
FROM EMPLOYEES
WHERE EMPLOYEE_ID IN (134,159,183);

-- 20. write a SQL query to find those employees whose salary is in the range
...
-- 23. write a SQL query to find those employees who get fifth-highest salary.
-- Return all the fields of the employees.
SELECT MIN(SALARY)
